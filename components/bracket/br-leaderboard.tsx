...
export default function BRLeaderboard({ 
  ...
  const leaderboard: Record<string, { id: string; name: string; wwcd: number; kills: number; total: number; matchesPlayed: number }> = {}

  participants.forEach(p => {
    leaderboard[p.id] = { id: p.id, name: p.name, wwcd: 0, kills: 0, total: 0, matchesPlayed: 0 }
  })
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-900 text-slate-400 font-bold text-xs uppercase">
            <tr>
              ...
              <th className="px-6 py-3 text-center text-white font-black text-base bg-white/5">PTS</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800">
            {sortedLeaderboard.map((team, idx) => (
              <tr key={team.id} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="px-4 py-3 text-center text-yellow-500 font-bold">{team.wwcd > 0 ? team.wwcd : '-'}</td>
                <td className="px-4 py-3 text-center text-red-400">{team.kills}</td>
                <td className="px-4 py-3 text-center text-slate-500">{team.matchesPlayed}</td>
                <td className="px-6 py-3 text-center font-black text-white text-lg bg-white/5">{team.total}</td>
              </tr>
            ))}
          </tbody>
        </table>
