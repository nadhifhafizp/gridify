...
export default function BracketVisualizer({ 
  ...
  const groupRounds = (matchList: any[]) => {
    ...
    return Object.keys(rounds).sort((a, b) => parseInt(a) - parseInt(b)).map(k => rounds[k])
  }
  ...
  const championName = championId 
    ? (championId === finalMatch?.participant_a_id ? finalMatch?.participant_a?.name : finalMatch?.participant_b?.name)
    : null
          ...
          {ubRounds.map((roundMatches: any[], idx) => (
            <div key={idx} className="flex flex-col justify-around gap-8 self-stretch">
              <div className="text-center mb-2">
                <span className="text-xs text-slate-500 font-mono">Round {idx + 1}</span>
              </div>
        ...
        <div className="border-t border-slate-800 pt-8">
          ...
          <div className="flex gap-12 min-w-max px-4">
            {lbRounds.map((roundMatches: any[], idx) => (
              <div key={idx} className="flex flex-col justify-around gap-8 self-stretch">
                <div className="text-center mb-2">
                  <span className="text-xs text-slate-500 font-mono">LB Round {idx + 1}</span>
                </div>
                <div className="flex flex-col justify-center gap-8 h-full">
                  {roundMatches.sort((a, b) => a.match_number - b.match_number).map(m => (
                    // FIX: Teruskan isReadOnly ke sini
