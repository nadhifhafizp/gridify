import { Shield, Swords, Medal, CheckCircle2 } from 'lucide-react'
import LeagueMatchCard from './league-match-card'
import AdvanceButton from './advance-button'

type Stat = {
  id: string, name: string, group: string, played: number, won: number, drawn: number, lost: number, gf: number, ga: number, gd: number, points: number
}
...
export default function StandingsTable({ 
  ...
  participants, 
  tournamentId,
  isReadOnly = false // FIX: Tambahkan default false
}: { 
  matches: any[], 
  participants: any[], 
  tournamentId: string,
  isReadOnly?: boolean // FIX: Tambahkan di definisi tipe
}) {
  ...
  participants.forEach(p => {
    ...
    const stat = { id: p.id, name: p.name, group: groupName, played: 0, won: 0, drawn: 0, lost: 0, gf: 0, ga: 0, gd: 0, points: 0 }
  ...
  matches.forEach(m => {
    if (m.status === 'COMPLETED' || (m.scores && (m.scores.a > 0 || m.scores.b > 0))) {
      ...
      if (pA && pB) {
        pA.played++; pA.gf += sA; pA.ga += sB; pA.gd += (sA - sB);
        pB.played++; pB.gf += sB; pB.ga += sA; pB.gd += (sB - sA);
        if (sA > sB) { pA.won++; pA.points += 3; pB.lost++; }
        else if (sA < sB) { pB.won++; pB.points += 3; pA.lost++; }
        else { pA.drawn++; pA.points += 1; pB.drawn++; pB.points += 1; }
      }
    }
  })
    ...
    <div className="space-y-12">
                ...
                <tbody className="divide-y divide-slate-800">
                  {groupStandings.map((team, index) => (
                    <tr key={team.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-3 py-4 text-center font-mono text-slate-400">{team.gd > 0 ? `+${team.gd}` : team.gd}</td>
                      <td className="px-6 py-4 text-center font-bold text-base text-indigo-400">{team.points}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="pt-8 border-t border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {matches.map((match) => (
            // FIX: Teruskan isReadOnly ke komponen kartu
            <LeagueMatchCard key={match.id} match={match} isReadOnly={isReadOnly} />
          ))}
        </div>
      ...
      {/* FIX: Sembunyikan tombol Advance jika sedang mode ReadOnly (Halaman Publik) */}
      {isLeagueFinished && !isReadOnly && (
