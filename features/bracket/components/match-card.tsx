...
export default function MatchCard({
  ...
  // --- LOGIC BARU ---
  // 1. Cek apakah kedua peserta sudah ada (lolos dari babak sebelumnya)
  const isMatchReady = p1 && p2;

  // 2. User hanya bisa edit jika:
  //    - Mode Admin (!isReadOnly)
  //    - DAN Match sudah siap (kedua tim ada)
  ...
  return (
    <>
      <div
        ...
        // Hanya buka modal jika canEdit true
        ...
        <div
          className={`border rounded-lg overflow-hidden shadow-lg transition-all relative z-10 ${
            canEdit
              ...
              : "border-slate-700 bg-slate-900/60 cursor-default" // Cursor default jika belum siap
          }`}
        >
          {/* Edit Overlay: Hanya muncul jika canEdit true */}
          {canEdit && (
              ...
              <span
                className={`text-sm font-medium truncate ${
                  winnerId && winnerId === p1?.id
                    ? "text-white"
                    : "text-slate-400"
                } ${!p1 ? "text-slate-600 italic" : ""}`} // Style italic jika TBD
              >
                {p1?.name || "Waiting..."} 
              </span>
              ...
              <span
                className={`text-sm font-medium truncate ${
                  winnerId && winnerId === p2?.id
                    ? "text-white"
                    : "text-slate-400"
                } ${!p2 ? "text-slate-600 italic" : ""}`} // Style italic jika TBD
              >
                {p2?.name || "Waiting..."}
              </span>
