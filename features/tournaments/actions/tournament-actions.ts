...
const CreateTournamentSchema = z.object({
  ...
  // FIX 1: Hapus errorMap custom untuk menghindari konflik tipe TypeScript
  formatType: z.enum([
    "SINGLE_ELIMINATION",
    "DOUBLE_ELIMINATION",
    "ROUND_ROBIN",
...
export async function createTournamentAction(formData: FormData) {
  ...
  if (!validatedFields.success) {
    return {
      success: false,
      // FIX 2: Gunakan .issues[0].message (bukan .errors) untuk properti ZodError yang valid
  ...
  try {
    ...
    if (stageError) {
      ...
      throw new Error("Gagal membuat format stage: " + stageError.message);
    }
  } catch (error: any) {
    return { success: false, error: error.message };
  }

  // 3. Redirect (DI LUAR TRY-CATCH agar tidak dianggap error)
  if (tournamentId) {
