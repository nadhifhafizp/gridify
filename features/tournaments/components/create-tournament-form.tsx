...
const getFormatDesc = (formatId: string, genre?: string) => {
  if (formatId === "HYBRID_UCL") {
    if (genre === "MOBA")
      return "Fase Grup lalu lanjut ke Upper/Lower Bracket (Standar M-Series/MPL).";
    return "Fase Grup lalu lanjut ke Fase Gugur (Standar Piala Dunia/UCL).";
  }
  if (formatId === "DOUBLE_ELIMINATION")
    return "Sistem kalah 2x eliminasi, dengan Upper & Lower Bracket.";
  if (formatId === "SINGLE_ELIMINATION")
    return "Kalah sekali langsung gugur, sistem knockout murni.";
  if (formatId === "ROUND_ROBIN")
    return "Semua tim bertemu semua tim, sistem poin seperti liga.";
  if (formatId === "BATTLE_ROYALE")
    return "Sistem poin kumulatif dari multiple match days.";
      ...
      try {
        ...
        if (result?.error) {
          ...
          toast.success("Tournament berhasil dibuat!", {
            description: "Redirecting ke dashboard...",
        ...
        {!selectedGame ? (
          <div className="text-center py-12 sm:py-16">
            ...
            <p className="text-sm text-slate-600 mt-1">
              untuk melihat format yang tersedia
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {availableFormats.map((format) => {
              const isRecommended = format.recommendedFor?.includes(
                selectedGame.genre
              );
                ...
                <label
                  ...
                  className={`cursor-pointer relative p-4 rounded-xl border-2 transition-all duration-200 group ${
                    isSelected
                      ...
                      : "border-slate-700/50 bg-slate-900/30 hover:border-slate-600 hover:bg-slate-800/50 hover:scale-[1.01]"
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  {isRecommended && (
                    <span className="absolute top-3 right-3 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 uppercase tracking-wide">
                      Direkomendasikan
                    </span>
                  )}
                  ...
                  <div className="flex items-start gap-3">
                    <div
                      className={`p-2.5 rounded-lg shrink-0 transition-colors ${
                        isSelected
                          ...
                          : "bg-slate-800/50 text-slate-400 group-hover:text-white group-hover:bg-slate-700"
                      }`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <span
                        className={`block font-bold text-sm sm:text-base mb-1 ${
                          isSelected ? "text-white" : "text-slate-200"
                        }`}
                      >
                        {label}
                      </span>
                      <span className="text-xs sm:text-sm text-slate-400 leading-relaxed line-clamp-2">
                        {desc}
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {selectedGame.genre === "SPORTS"
                  ? "Konfigurasi pertandingan sepak bola"
                  : "Konfigurasi match esports"}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            {/* OPSI SPORTS - Home & Away */}
            {selectedGame.genre === "SPORTS" && (
                ...
                <label className="flex items-center justify-between p-4 rounded-xl border border-slate-700/50 bg-slate-900/30 cursor-pointer hover:border-slate-600 hover:bg-slate-800/50 transition-all group">
                  <div className="flex-1">
                    ...
                    <span className="text-xs sm:text-sm text-slate-400">
                      Setiap pertandingan dimainkan 2x (kandang & tandang),
                      pemenang ditentukan dari agregat skor.
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {["1", "3", "5"].map((bo) => (
                    <button
                      ...
                      className={`relative py-3 sm:py-4 rounded-xl border-2 font-bold text-sm sm:text-base transition-all duration-200 ${
                        matchConfig.bestOf === bo
                          ? "bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-500/30 scale-105"
                          : "bg-slate-900/50 border-slate-700/50 text-slate-400 hover:text-white hover:border-slate-600 hover:scale-105"
                      }`}
                    >
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <span className="text-[10px] text-slate-500">
                          {bo === "1"
                            ? "Single Game"
                            : `First to ${Math.ceil(parseInt(bo) / 2)}`}
                        </span>
                      </div>
                      {matchConfig.bestOf === bo && (
